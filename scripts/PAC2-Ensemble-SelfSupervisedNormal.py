...
def PAC2Ensemble(dataSource=tf.keras.datasets.fashion_mnist, NPixels=28, algorithm=0, num_ensemble_models=1, batch_size=100, num_epochs=50, num_hidden_units=20):
    """ Run experiments for Ensemble, PAC^2-Ensemble and PAC^2_T-Ensemble algorithms for the self-supervised classification task with a Normal data model.
        Args:
    ...
    def model(NHIDDEN, x):
        ...
        W_out = tf.Variable(tf.random_normal([NHIDDEN, NPixels], 0.0, 0.1, dtype=tf.float32, seed=1))
        b_out = tf.Variable(tf.random_normal([1, NPixels], 0.0, 0.1, dtype=tf.float32, seed=1))
...
iter=100
batch=100
text_file = open("./results/output-PAC2-Ensemble-SelfSupervisedNormal.txt", "w")
