...
def PAC2Ensemble(dataSource = tf.keras.datasets.fashion_mnist, NLabels=10, NPixels=28, algorithm=0, num_ensemble_models=1, batch_size=100, num_epochs=50, num_hidden_units = 20):
    """ Run experiments for Ensemble, PAC^2-Ensemble and PAC^2_T-Ensemble algorithms for the supervised classification task.
        Args:
    ...
    def model(NHIDDEN, x):
        W = tf.Variable(tf.random_normal([NPixels*NPixels, NHIDDEN], 0.0, 0.1, dtype=tf.float32, seed=1))
        b = tf.Variable(tf.random_normal([1, NHIDDEN], 0.0, 0.1, dtype=tf.float32, seed=1))

        W_out = tf.Variable(tf.random_normal([NHIDDEN, NLabels], 0.0, 0.1, dtype=tf.float32, seed=1))
        b_out = tf.Variable(tf.random_normal([1, NLabels], 0.0, 0.1, dtype=tf.float32, seed=1))
    ...
    for i in range(K):
        ...
        t.append(tf.expand_dims(py.distribution.log_prob(y_batch),axis=1))
        tpy.append(py)
    ...
    if K>1:
        max = tf.constant(-math.log(0.9999))
    ...
    for i in range(num_epochs+1):
        ...
        for j in range(N // M):
            batch_x = np.reshape(x_batches[j], [batch_size, -1]).astype(np.float32)
            batch_y = np.reshape(y_batches[j],[batch_size,]).astype(np.float32)
    ...
    for j in range(N // M):
        batch_x = np.reshape(x_batches[j], [M, -1]).astype(np.float32)
        batch_y = np.reshape(y_batches[j], [M,]).astype(np.float32)
        y_pred_list = []
        for i in range(K):
            y_pred_list.append(tf.expand_dims(tpy[i].distribution.log_prob(batch_y), axis=1))
        y_preds = tf.concat(y_pred_list, axis=1)
        score = tf.reduce_sum(tf.math.reduce_logsumexp(y_preds, axis=1) - tf.log(K + 0.0))
        score = sess.run(score,feed_dict={x_batch: batch_x})
        NLL = NLL + score
        if verbose:
...
iter=100
batch=100
text_file = open("./results/output-PAC2-Ensemble-Supervised.txt", "w")

text_file.write(str(PAC2Ensemble(dataSource= tf.keras.datasets.fashion_mnist, NLabels=10, NPixels=28, algorithm=0, num_ensemble_models=1, batch_size=batch, num_epochs=iter, num_hidden_units= 20)) + "\n")
text_file.flush()
text_file.write(str(PAC2Ensemble(dataSource= tf.keras.datasets.fashion_mnist, NLabels=10, NPixels=28, algorithm=1, num_ensemble_models=2, batch_size=batch, num_epochs=iter, num_hidden_units= 20)) + "\n")
text_file.flush()
text_file.write(str(PAC2Ensemble(dataSource= tf.keras.datasets.fashion_mnist, NLabels=10, NPixels=28, algorithm=2, num_ensemble_models=2, batch_size=batch, num_epochs=iter, num_hidden_units= 20)) + "\n")
text_file.flush()
text_file.write(str(PAC2Ensemble(dataSource= tf.keras.datasets.fashion_mnist, NLabels=10, NPixels=28, algorithm=1, num_ensemble_models=3, batch_size=batch, num_epochs=iter, num_hidden_units= 20)) + "\n")
text_file.flush()
text_file.write(str(PAC2Ensemble(dataSource= tf.keras.datasets.fashion_mnist, NLabels=10, NPixels=28, algorithm=2, num_ensemble_models=3, batch_size=batch, num_epochs=iter, num_hidden_units= 20)) + "\n")
text_file.flush()


text_file.write(str(PAC2Ensemble(dataSource= tf.keras.datasets.cifar10, NLabels=10, NPixels=32, algorithm=0, num_ensemble_models=1, batch_size=batch, num_epochs=iter, num_hidden_units= 20)) + "\n")
text_file.flush()
text_file.write(str(PAC2Ensemble(dataSource= tf.keras.datasets.cifar10, NLabels=10, NPixels=32, algorithm=1, num_ensemble_models=2, batch_size=batch, num_epochs=iter, num_hidden_units= 20)) + "\n")
text_file.flush()
text_file.write(str(PAC2Ensemble(dataSource= tf.keras.datasets.cifar10, NLabels=10, NPixels=32, algorithm=2, num_ensemble_models=2, batch_size=batch, num_epochs=iter, num_hidden_units= 20)) + "\n")
text_file.flush()
text_file.write(str(PAC2Ensemble(dataSource= tf.keras.datasets.cifar10, NLabels=10, NPixels=32, algorithm=1, num_ensemble_models=3, batch_size=batch, num_epochs=iter, num_hidden_units= 20)) + "\n")
text_file.flush()
text_file.write(str(PAC2Ensemble(dataSource= tf.keras.datasets.cifar10, NLabels=10, NPixels=32, algorithm=2, num_ensemble_models=3, batch_size=batch, num_epochs=iter, num_hidden_units= 20)) + "\n")
text_file.flush()
